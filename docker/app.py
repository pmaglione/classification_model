...
#from model.model import CrowdJob, FigureEight
...
def post():
    ...
    #crowd_job = CrowdJob(api_key, FigureEight(api_key), classification_fn, cost_ratio, items_per_worker, title, instructions)
...
def get_formatted_items_votes(crowd_job):
    # reformat to {item_id: {worker_id:[vote]}}
    total_votes = {}
    for item in crowd_job.items:
        for worker_id, vote in item.votes.items():
            total_votes[item.internal_id][worker_id] = [vote]

    return total_votes

@app.route('/get_assigment/<int:job_id>/<int:worker_id>', methods=['GET'])
@api.doc(params={
    'job_id': 'Job ID',
    'worker_id': 'Worker Uuid'
})
def get_assigment(job_id, worker_id):
    crowd_job = CrowdJob(mongo, job_id)
    free_items = crowd_job.get_items_by_state(Item.STATE_FREE)
    item = RandomStrategy().get_assignment(worker_id, free_items)
    crowd_job.assign_item(item['internal_id']) #change state to assigned

    response = {
                    "id": item['internal_id'],
                    "attributes": item['attributes_names'],
                    "content": item['values']
                }
    return jsonify(response)

@app.route('/submit_votes/<int:job_id>/<int:worker_id>/<int:item_id>/<int:vote>', methods=['POST'])
@api.doc(params={
    'job_id': 'Job ID',
    'worker_id': 'Worker Uuid',
    'item_id': 'Item ID',
...
def submit_votes(job_id, worker_id, item_id, vote):
    crowd_job = CrowdJob(mongo, job_id)
